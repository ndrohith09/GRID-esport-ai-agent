...
import { TOOL_TYPES, type AIMessage, type ChatHistory } from "./Tools/type";
...
import { createNewChat, getTeamChats } from "../../api/chat";
import { useParams } from "react-router-dom";
import ImpactAnalysis from "../Player/PlayerStrengthWeak";
import TeamImpactAnalysis from "../Dashboard/TeamStrengthWeak";
import ScoutingReportCard from "./Tools/ScoutingReportCard";
...
const [loading, setLoading ] = useState<boolean>(false);
  ...
  const submitHandler = (message: string, llm_input: string) => {
    setLoading(true);
    createNewChat(team_id ?? '', message, llm_input).then((data) => {
      setLoading(false);
      getTeamChats()
    .then((res) => { 
      setMessages(res);
  ...
  return text
    .replace(/(@\w+)/g, (match) => {
      return `<span class="bg-green-200 px-1 rounded">${match}</span>`;
    })
    .replace(/(\$\w+)/g, (match) => {
      return `<span class="bg-red-200 px-1 rounded">${match}</span>`;
    })
.replace(/(#\w+)/g, (match) => {
  return `<span class="bg-blue-200 px-1 rounded">${match}</span>`;
});
};
  ...
  return (
    ...
    <div className="h-full w-full bg-white flex flex-col relative overflow-hidden">
                ...
                prose-pre:bg-gray-900 prose-pre:text-white prose-pre:p-2"
                      >
                        ...
                        <ReactMarkdown remarkPlugins={[remarkGfm]}>
                          {JSON.parse(msg.message)?.response} 
                        </ReactMarkdown>

                            {JSON.parse(msg.message)?.artifacts.tool_name === TOOL_TYPES.PLAYER_VS_PLAYER_PROBABILITY && <WinMeter artifacts={JSON.parse(msg.message)?.artifacts}  />}


                        {/* TEAM VS TEAM Probablity */}
                        {JSON.parse(msg.message)?.artifacts.tool_name === TOOL_TYPES.TEAM_VS_TEAM_PROBABILITY && <TeamTeamWinMeter artifacts={{
                          A_kills_mean: 0,
                         ...
                         {  JSON.parse(msg.message)?.artifacts.tool_name === TOOL_TYPES.PLAYER_PROBABILITY && <PlayerWinProbabilityGauge  artificats={{
                          monte_carlo : {
                              "scenario": {
                         ...
                         { JSON.parse(msg.message)?.artifacts.tool_name === TOOL_TYPES.TEAM_PROBABILITY && <TeamWinProbabilityGauge
                           artificats={{
                              "deaths_mean": 184.1673,
                              "kill_diff_mean": -4.2366,
                              "kills_mean": 179.9307,
                         ...
                         />}
                           ...
                           {/* { JSON.parse(msg.message)?.artifacts.tool_name === TOOL_TYPES.GET_PLAYER_SERIES_DATA && 
                           <div className="max-w-xl">
                      <ImpactAnalysis weakness={JSON.parse(msg.message)?.artifacts.series_weaknesses} strengths={JSON.parse(msg.message)?.artifacts.series_strengths} />
                           </div>                           
                           }
                   */}
                  {/* { JSON.parse(msg.message)?.artifacts.tool_name === TOOL_TYPES.GET_TEAM_OVERALL_DATA && 
                           <div className="max-w-xl">
                        <TeamImpactAnalysis weakness={JSON.parse(msg.message)?.artifacts.weaknesses} strengths={JSON.parse(msg.message)?.artifacts.strengths} />                    
                           </div>                           
                           } */}
                   
                  { JSON.parse(msg.message)?.artifacts.tool_name === TOOL_TYPES.GENERATE_SCOUTING_REPORT && 
                           <div className="max-w-xl">
                            <ScoutingReportCard artifacts={JSON.parse(msg.message)?.artifacts} />
                           </div>                           
                           }
                      ...
                      <>
                      <div dangerouslySetInnerHTML={{ __html: highlightText(msg.message) }} />
                      </>
                    )}
                  </div>  
                </div>
      ...
      <ChatInput loading={loading} submitHandler={submitHandler} />
    </div>
  );
};
